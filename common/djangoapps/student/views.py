...
def create_account(request, post_override=None):  # pylint: disable-msg=too-many-statements
    ...
    if not (settings.FEATURES.get('AUTOMATIC_AUTH_FOR_TESTING')):
        ...
        try:
            ...
            log.warning('Unable to send activation email to user from "{from_address}"'.format(from_address=from_address), exc_info=True)
...
def reactivation_email_for_user(user):
    ...
    try:
        ...
        log.warning('Unable to send reactivation email from "{from_address}"'.format(from_address=settings.DEFAULT_FROM_EMAIL), exc_info=True)
        return JsonResponse({
            "success": False,
